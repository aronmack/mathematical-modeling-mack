//nvcc HW11QR.cu -o bounce -lglut -lm -lGLU -lGL																													
...
{
	float inOut;
	float p,pSphereReduction;		//p and pSphereReduction are in place of kSphere and kSphereReduction
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < SphereDiameter)
			{
				...
				if(inOut < 0.0) magnitude = p*intersectionArea; // If inOut is negative the sphere are converging.
				else magnitude = pSphereReduction*p*intersectionArea; // If inOut is positive the sphere are diverging.
